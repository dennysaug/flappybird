...
*/
   ...
   function showSplash()
   {
      ...
      console.log('ENTER: Remove os canos');
      console.log('ESC: Pausa o game');
      console.log('SHIFT: Continua o game');
      console.log('X: cheat para aumentar a pontuação (score)');
   }
   ...
   $(document).keydown(function(e){
      ...
      //NOVA FUNÇÃO ADICIONADA 1: A TECLA <ENTER> REMOVE OS CANOS #DENNYS
      if(e.keyCode == 13)
      {
         removePipes();
      }
      //NOVA FUNÇÃO ADICIONADA 2: A TECLA <ESC> PAUSA O GAME #DENNYS
      if(e.keyCode == 27)
      {
         pausa();
      }

      //NOVA FUNÇÃO ADICIONADA 3: A TECLA <SHIFT> CONTINUA (TIRA O PAUSED) O GAME #DENNYS
      if(e.keyCode == 16)
      {
         continua();
      }
   ...
   {
      // Existe algum cano para remover?
      $(".pipe").filter(function() { return $(this).position().left <= 600; }).remove();
      pipes.splice(0, 3);
   }
   ...
   {
      $(".animated").css('animation-play-state', 'running');
      $(".animated").css('-webkit-animation-play-state', 'runngin');
   }
